            ...
            {dummy.map((article, index) => (
                ...
                >
                    <Box sx={{
                        ...
                        <Typography component="div" sx={{ flexGrow: 1, color: '#777777', fontWeight: 600, fontSize: '1.3rem', textAlign: 'justify', boxSizing: 'border-box' }}>
                            {article.subject}
                        </Typography>
                        <Box sx={{ mb: 1 }}>
                            <Chip icon={<FaceIcon />} label={article.author} variant="outlined" sx={{ mr: '6px', fontWeight: 600 }} />
                            {article.tag.map((item, tagindex) => (
                                <Chip key={`discussion-article-${index}-tags-${tagindex}`} label={item} color="primary" variant="outlined" sx={{ mr: '6px' }} />
                            ))}
                        </Box>
                    </Box>
                    <Box sx={{
